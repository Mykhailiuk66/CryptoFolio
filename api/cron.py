...
def create_or_update_coin_exchange_info_bulk(infos_list: List[Dict[str, any]]): # [ticker_info: dict, exchange_name: str, date: date]
    bulk_instances = []
    
    for info in infos_list:
        ticker_info = info['ticker_info']
        coin, c_created = Coin.objects.get_or_create(short_name=ticker_info['coin']) 
        exchange, ex_created = Exchange.objects.get_or_create(slug=info['exchange_name'], defaults={'name': info['exchange_name'].capitalize()})
