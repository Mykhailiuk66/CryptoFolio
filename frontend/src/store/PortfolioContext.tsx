...
export const PortfolioProvider = ({ children }: DefaultProviderProps) => {
	const [portfolios, setPortfolios] = useState<PortfolioType[]>([]);
	const [portfolioSnapshots, setPortfolioSnapshots] = useState<
		PortfolioSnapshot[]
	>([]);
	const [selectedPortfolio, setSelectedPortfolio] = useState<string>();
	const [selectedPortfolioHolding, setSelectedPortfolioHolding] =
		useState<string>();
	...
	const addPortfolio = useCallback(
		async (submittedPortfolio: PortfolioFormType) => {
			try {
				const response = await fetch(`/api/portfolios/`, {
					method: "POST",
					headers: {
		...
		async (submittedPortfolio: PortfolioFormType) => {
			try {
				const response = await fetch(
					`/api/portfolios/${selectedPortfolio}/`,
					{
						method: "PUT",
						headers: {
							"Content-Type": "application/json",
	...
	const deletePortfolio = useCallback(async () => {
		try {
			...
			if (response.ok) {
				setPortfolios((prevState) =>
					prevState.filter((p) => p.id !== selectedPortfolio)
				);
	...
	const fetchPortfolioSnapshots = useCallback(async () => {
		try {
			const response = await fetch(
				`/api/portfolios/${selectedPortfolio}/snapshots/`,
				{
					method: "GET",
					headers: {
						"Content-Type": "application/json",
	...
	const removePortfolioHolding = useCallback(
		async (portfolioHoldingId: string) => {
			try {
				const response = await fetch(
					`/api/portfolio-holdings/${portfolioHoldingId}/`,
					{
						method: "DELETE",
						headers: {
							"Content-Type": "application/json",
	...
	const addPortfolioHolding = useCallback(
		async (formData: PortfolioHoldingForm) => {
			try {
	...
	const editPortfolioHolding = useCallback(
		async (formData: PortfolioHoldingForm, holdingId: string) => {
			try {
				const response = await fetch(
